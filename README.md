...
<p align="center">
  <strong>Grok Build 配置模板 · 一键安装 · 交互向导</strong>
</p>

<p align="center">
  把 <a href="https://x.ai">Grok CLI</a> 接到你自己的 OpenAI 兼容网关：<br/>
  装 CLI → 填别名 / <code>base_url</code> / <code>api_key</code> → 写入 <code>~/.grok/config.toml</code><br/>
  可选原生 Search Tool · 可选 <a href="https://github.com/Autsunset/grok-search">grok-search</a> skill
</p>
...
<p align="center">
  ...
  <a href="#联网能力"><img src="https://img.shields.io/badge/search-native%20%2B%20skill-8b5cf6?style=flat-square" alt="search" /></a>
...
| 上手 | 配置 | 联网 | 其他 |
|------|------|------|------|
| [一键安装](#一键安装) | [配置说明](#配置说明) | [联网能力](#联网能力) | [故障排查](#故障排查) |
| [安装后验证](#安装后验证) | [常见自定义](#常见自定义) | [原生 Search Tool](#原生-search-tool) | [安全建议](#安全建议) |
| [项目结构](#项目结构) | [字段速查](#字段速查) | [grok-search skill](#grok-search-skill) | [许可证与免责](#许可证与免责) |
| [分步安装](#分步安装) | | | |
...
**推荐路径。** 一条命令完成 CLI 安装 + 配置向导。

```text
  bootstrap.sh
       │
       ├─ 1. 安装 Grok Build CLI      （已有 grok 则跳过）
       ├─ 2. 下载 config.toml + 向导   （默认临时目录，结束后清理）
       ├─ 3. 交互写入 ~/.grok/config.toml
       └─ 4. 可选 grok-search skill   （search / fetch / map）
```
...
| 2 | **base_url** | `http://` 或 `https://` 开头 |
| 3 | **api_key** | 输入可见，便于核对；可选二次确认 |
| 4 | **Search Tool** | 可选；原生 `web_search` / `x_search` |
| 5 | **确认写入** | 已有配置先备份为 `config.toml.bak.<时间戳>` |
| 6 | **grok-search** | 可选；[Autsunset/grok-search](https://github.com/Autsunset/grok-search) |
...
`default` 是本地昵称，**不是** API 的模型 ID。安装脚本会同步改写 `default` 与 `[model.名称]`。
...
| 规则 | 要求 |
|------|------|
| **别名** | 字母开头，后接字母 / 数字 / `_` / `-`（如 `Steve`、`MyProxy`、`work-grok`） |
...
```text
  用户会话
      ...
      └─ grok-search skill ──► node scripts/{search,fetch,map}.js
                                配置：~/.config/grok-search/config.json
```
...
| **向导默认** | 关闭（`[y/N]`） | 未开原生时推荐装（`[Y/n]`） |

同一问题不要既 `web_search` 又跑 `search.js`，避免重复联网。

---

### 原生 Search Tool

向导询问 **「是否开启 Search Tool?」**。选 **y** 时写入：

```toml
[models]
default = "Steve"
web_search = "Steve"              # Build 的 web_search 指向本模型
...
| 场景 | 建议 |
|------|------|
| 网关已支持原生 `web_search` / `x_search`（如 grok2api Build 渠道） | 向导里 **开启** |
| 普通 OpenAI 兼容转发、不支持 Responses 搜索 | **关闭**（默认）；改用 skill |
...
| 脚本 | 能力 |
|------|------|
| `search.js` | Grok Chat / Responses 联网；可并行 Tavily / Firecrawl |
...
5. 选协议：`chat`（grok2api / NewAPI）或 `responses`（CPA / xAI 风格）
6. 搜索模型 ID（默认 `grok-4.3-fast` 或 `grok-4.5`）
7. 复用本次 `base_url` / `api_key` 写 `config.json`（可改）
...
Skill 装在 `~/.grok/skills/`，Grok 会自动发现；需要联网 / 读网页时模型可按 `SKILL.md` 调用脚本。
...
| 模型别名格式无效 | 旧脚本把提示混入输入 | 用最新 `install-config.sh`；合法例：`Steve`、`MyProxy`、`work-grok` |
| `base_url` 格式无效 | 缺协议或含空格 | 使用 `https://host/v1` |
| 连不上模型 | 地址 / 密钥 / 网络 | 查 `base_url`、`api_key`、服务商控制台 |
| 开启 Search 后报错 / 无搜索 | 网关不支持 Responses 或原生搜索 | 关掉 Search Tool，或换支持网关；或改用 **grok-search** |
| 对话里不联网 | 旧 session / 未开 Search | 新开 session；检查 `web_search` 与 `supports_backend_search`；或装 skill |
| grok-search 跳过 / 失败 | 无 Node / 版本低 / 无 git / npm 失败 | Node ≥ 18.17 + git；`cd ~/.grok/skills/grok-search && npm install` 后重跑向导 |
| `search.js` 401 / 404 / 422 | key、协议或模型不匹配 | 核对 `config.json` 的 `searchEndpoint` 与 `model`（chat 用快模型，responses 常用 `grok-4.5`） |
...
| 做 | 不做 |
|----|------|
| 安装脚本强制你重新填写密钥 | 把含真实 `api_key` 的配置提交到 Git |
